...
def _admin_login(client: TestClient) -> dict:
    """Create user via auto-provisioning (first user = admin) and return auth headers."""
    token = _make_jwt(email="admin@test.com")
...
class TestPerfEndpoint:
    ...
    def test_perf_no_data_returns_message(self, api_client: TestClient):
        ...
        headers = _admin_login(api_client)
        api_module._search_timings.clear()
        response = api_client.get("/api/perf", headers=headers)
...
class TestTimingMiddleware:
    ...
    def test_non_search_requests_not_recorded(self, api_client: TestClient):
        ...
        api_client.get("/api/perf", headers=headers)
